  ...
  >
    <el-card class="box-card lg:md:px-20 sm:px-0">
      <div class="lg:md:flex justify-between">
        <div class="flex">
          <div
            class="bg-yellow-400 inline h-10 w-10 text-center rounded-full sm:hidden lg:block"
          >
            <span class="i-mdi-building pt-5 text-4xl text-white"></span>
          </div>
          <div class="inline px-3">
            <!-- <p class="mt-0 mb-0 font-thin">SEARCH SUMMERY</p> -->
            <P class="font-bold mb-0 mt-0" v-if="availbleHotels">{{
              availbleHotels.destination
            }}</P>
            <div class="flex align-center" v-if="availbleHotels">
              <span class="text-xs font-bold mt-0 mb-0 text-slate-500"
                >from :{{ availbleHotels.checkIn }}</span
              >
              <!-- music-note-whole -->

              <span
                class="text-xs font-bold mt-0 px-1 text-slate-500 flex align-center"
              >
        ...
        <div>
          <router-link to="/" class="no-underline">
            <el-button type="success" class="px-10 my-5 text-xs font-bold"
              >Modify</el-button
            >
          </router-link>
        </div>
